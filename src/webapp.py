...
@app.route('/api/film-events')
def api_film_events():
    """Theater film ground truth: exact kill/death/medal times per match.
    Joined with each match's wall-clock start so consumers (e.g. MultiTwitch
    highlight reels) can map any event to a real-world instant:
    utc = start_epoch + time_ms/1000."""
    try:
        hours = min(2160, max(1, int(request.args.get('hours', 336))))
    except (TypeError, ValueError):
        hours = 336
    only_match = str(request.args.get('match_id') or '')
    try:
        with ENGINE.connect() as conn:
            rows = conn.execute(text(
                """
        ...
        logger.warning('film-events api failed: %s', exc)
        return jsonify({'matches': {}, 'error': 'film events unavailable'}), 200
